...
unit RatingsAppearanceCustomBottomU;

interface

uses
  System.Classes, System.SysUtils, FMX.Types, FMX.Controls, System.UITypes, FMX.Objects, FMX.ListView,
  FMX.ListView.Types, FMX.ListView.Appearances;

type

  TRatingsListItemBottomAppearanceNames = class
  public const
    ListItem = 'RatingListItemBottom';
  ...
  TRatingsImageBottomObjectAppearance = class(TImageObjectAppearance)
  private type
    TNotify = class(TComponent)
    private
      FOwner: TRatingsImageBottomObjectAppearance;
    protected
...
type
  ...
  TRatingsListItemBottomAppearance = class(TPresetItemObjects)
  public const
    cDefaultHeight = 80;
  private
    FText2: TTextObjectAppearance;
    FRatingsImage: TRatingsImageBottomObjectAppearance;
    FText3: TTextObjectAppearance;
    procedure SetText2(const Value: TTextObjectAppearance);
    procedure SetRatingsImage(const Value: TRatingsImageBottomObjectAppearance);
  ...
  published
    ...
    property RatingsImage: TRatingsImageBottomObjectAppearance read FRatingsImage write SetRatingsImage;
    property Accessory;
  end;

  TRatingsListItemDeleteAppearance = class(TRatingsListItemBottomAppearance)
  private const
  ...
  TRatingsListItemShowCheckAppearance = class(TRatingsListItemBottomAppearance)
  private const
...
constructor TRatingsListItemBottomAppearance.Create(const Owner: TControl);
begin
  inherited;
  ...
  FText2.Name := TRatingsListItemBottomAppearanceNames.Text2Name;
  ...
  Text2.DataMembers :=
    TObjectAppearance.TDataMembers.Create(
      TObjectAppearance.TDataMember.Create(
        ...
        Format('Data["%s"]', [TRatingsListItemBottomAppearanceNames.Text2Name])));   // Expression to access value from TListViewItem

  FText3 := TTextObjectAppearance.Create;
  FText3.Name := TRatingsListItemBottomAppearanceNames.Text3Name;
  ...
  Text3.DataMembers :=
    TObjectAppearance.TDataMembers.Create(
      TObjectAppearance.TDataMember.Create(
        ...
        Format('Data["%s"]', [TRatingsListItemBottomAppearanceNames.Text3Name])));   // Expression to access value from TListViewItem
  ...
  FRatingsImage := TRatingsImageBottomObjectAppearance.Create;
  FRatingsImage.Name := TRatingsListItemBottomAppearanceNames.RatingsImageName;
  FRatingsImage.DefaultValues.Visible := True;
  //FRatingsImage.DefaultValues.VertAlign := TListItemAlign.Trailing;
  FRatingsImage.DefaultValues.VertAlign := TListItemAlign.Center;
  ...
  FRatingsImage.DataMembers :=
    TObjectAppearance.TDataMembers.Create(
      TObjectAppearance.TDataMember.Create(
        ...
        Format('Data["%s"]', [TRatingsListItemBottomAppearanceNames.RatingsImageName])));   // Expression to access value from TListViewItem
...
constructor TRatingsListItemDeleteAppearance.Create(const Owner: TControl);
begin
  inherited;
...
constructor TRatingsListItemShowCheckAppearance.Create(const Owner: TControl);
begin
  inherited;
  GlyphButton.DefaultValues.ButtonType := cDefaultGlyph;
  GlyphButton.DefaultValues.Visible := True;
...
function TRatingsListItemBottomAppearance.DefaultHeight: Integer;
begin
  Result := cDefaultHeight;
end;

destructor TRatingsListItemBottomAppearance.Destroy;
begin
  FText2.Free;
  FText3.Free;
  FRatingsImage.Free;
...
procedure TRatingsListItemBottomAppearance.SetText2(
  const Value: TTextObjectAppearance);
begin
  FText2.Assign(Value);
end;

procedure TRatingsListItemBottomAppearance.SetText3(
  const Value: TTextObjectAppearance);
begin
  FText3.Assign(Value);
end;

procedure TRatingsListItemBottomAppearance.SetObjectData(
  const AListViewItem: TListViewItem; const AIndex: string;
  const AValue: TValue; var AHandled: Boolean);
begin
  if AIndex = TRatingsListItemBottomAppearanceNames.RatingsImageName then
  begin
    AHandled := True;
...
procedure TRatingsListItemBottomAppearance.UpdateImageSrcRect(const AListViewItem: TListViewItem);
var
...
begin
  LObject := AListViewItem.Objects.FindDrawable(TRatingsListItemBottomAppearanceNames.RatingsImageName) as TListItemImage;
  Assert(LObject <> nil);
  LValue := AListViewItem.Data[TRatingsListItemBottomAppearanceNames.RatingsImageName];
  LRating := 0;
...
procedure TRatingsListItemBottomAppearance.SetRatingsImage(const Value: TRatingsImageBottomObjectAppearance);
begin
  FRatingsImage.Assign(Value);
end;

function TRatingsListItemBottomAppearance.GetGroupClass: TPresetItemObjects.TGroupClass;
begin
  Result := TRatingsListItemBottomAppearance;
end;

procedure TRatingsListItemBottomAppearance.UpdateSizes(const ItemSize: TSizeF);
var
  LTextOffset: Single;
begin
  BeginUpdate;
  try
    ...
    RatingsImage.InternalWidth := ItemSize.Width;
  finally
    EndUpdate;
  end;
  ...
  { TRatingsImageBottomObjectAppearance }

procedure TRatingsImageBottomObjectAppearance.CreateObject(const AListViewItem: TListViewItem);
var
  LObject: TListItemImage;
begin
  inherited;
  ...
  begin
    ...
    (Owner as TRatingsListItemBottomAppearance).UpdateImageSrcRect(AListViewItem);
  end;
end;

destructor TRatingsImageBottomObjectAppearance.Destroy;
begin
  FNotify.Free;
  inherited;
end;

procedure TRatingsImageBottomObjectAppearance.ResetObject(const AListViewItem: TListViewItem);
begin
  inherited;
  (Owner as TRatingsListItemBottomAppearance).UpdateImageSrcRect(AListViewItem);
end;

procedure TRatingsImageBottomObjectAppearance.SetRatingsImage(const Value: TImage);
begin
...
procedure TRatingsImageBottomObjectAppearance.SetRatingsCount(const Value: Integer);
begin
  FRatingsCount := Value;
  DoChange;
end;

{ TRatingsImageBottomObjectAppearance.TNotify }

procedure TRatingsImageBottomObjectAppearance.TNotify.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
...
const
  sThisUnit = 'RatingsAppearanceCustomBottomU';     // Will be added to the uses list when appearance is used


initialization
  // RatingsListItem group
  TAppearancesRegistry.RegisterAppearance(
    TRatingsListItemBottomAppearance, TRatingsListItemBottomAppearanceNames.ListItem,
    [TRegisterAppearanceOption.Item], sThisUnit);
  TAppearancesRegistry.RegisterAppearance(
    TRatingsListItemDeleteAppearance, TRatingsListItemBottomAppearanceNames.ListItemDelete,
    [TRegisterAppearanceOption.ItemEdit], sThisUnit);
  TAppearancesRegistry.RegisterAppearance(
    TRatingsListItemShowCheckAppearance, TRatingsListItemBottomAppearanceNames.ListItemCheck,
    [TRegisterAppearanceOption.ItemEdit], sThisUnit);
finalization
  TAppearancesRegistry.UnregisterAppearances(
    TArray<TItemAppearanceObjectsClass>.Create(
      TRatingsListItemBottomAppearance, TRatingsListItemDeleteAppearance,
